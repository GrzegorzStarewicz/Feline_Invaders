...
uint16_t get_entropy() {
	uint16_t seed = 0;
	for (int i = 0; i < 16; i++) {
		ADCSRA |= (1 << ADSC); // Start ADC conversion
		while (ADCSRA & (1 << ADSC)); // Wait for conversion to finish
		seed = (seed << 1) | (ADC & 0x01); // Extract the least significant bit
	}
	return seed;
}
...
void check_i_eliminacja()
{
	...
	// score na podstawie liczby 1 w eliminacji
	uint8_t count = 0;
	for (count = 0; eliminacja; count++) {
...
{	
	...
	if(co2){
		...
		// spawnowanie wrogow
		wrogowieA |= (rand()%trudnosc==0)<<15;
		wrogowieB |= (rand()%trudnosc==0)<<15;
