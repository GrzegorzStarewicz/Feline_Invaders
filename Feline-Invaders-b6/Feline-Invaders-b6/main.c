/*
 * Feline-Invaders-b6.c
 *
 * Created: 16.07.2026 20:17:00
 * Author : £ka
 */ 
...
// znaki
#define znGracz 0b00000000
#define znWrog 0b00000001
#define znPocisk '='
#define znNic ' '
#define znMegaPrz 0b00000010
#define znMegaTyl 0b00000011

//CUSTOM SPRITE'Y
uint8_t enemy[] = {0x02,0x03,0x1F,0x0A,0x0A,0x1F,0x03,0x02};
uint8_t nasz_statek[] = {0x18,0x1C,0x0E,0x1D,0x1D,0x0E,0x1C,0x18};
uint8_t big_enem_front[] = {0x00,0x00,0x07,0x1F,0x1F,0x07,0x00,0x00};
uint8_t big_enem_back[] = {0x02,0x0F,0x1E,0x17,0x17,0x1E,0x0F,0x02};
// KONIEC TABELEK
...
//po definicji lcd_zapis
void lcd_custom_znak(uint8_t nr,const uint8_t *cust_znak){

	for(uint8_t i = 0;i < 8;i++){
		sterRS &= ~(1<<RS); // bedziemy podawac instrukcje
		sterRW &= ~(1<<RW);
		LCD_zapis(0x40 + nr*8 +i);
		sterRS |= (1<<RS);
		LCD_zapis(cust_znak[i]);
	}
...
void custom_znaki_init(){

lcd_custom_znak(0,nasz_statek);
lcd_custom_znak(1,enemy);
lcd_custom_znak(2,big_enem_front);
lcd_custom_znak(3,big_enem_back);
}
